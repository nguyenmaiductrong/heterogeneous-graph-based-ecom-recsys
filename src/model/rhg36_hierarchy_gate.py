...
if __name__ == "__main__":
    ...
    print("=" * 55)
    print("ACCEPTANCE CRITERIA — RHG-36")
    print("=" * 55)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    emb_model  = HeteroEmbedding(NUM_USERS_ACTUAL, NUM_PRODUCTS_ACTUAL, dim=128).to(device)
    ...
    emb_view,     _ = emb_model(u_view,  p_view)
    emb_cart,     _ = emb_model(u_cart,  p_cart)
    emb_purchase, _ = emb_model(u_purch, p_purch)
    ...
    assert emb_model.user_emb.weight.grad    is not None, "Grad user_emb bị None!"
    ...
    print(f" Gradient    — user_emb grad norm: {emb_model.user_emb.weight.grad.norm():.4f}")

    print("\n" + "=" * 55)
    print("RHG-36 PASSED — HierarchyGate sẵn sàng")
    print("=" * 55)
