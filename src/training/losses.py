...
def sampled_softmax_loss(
    ...
    One softmax over ``1 + num_neg`` logits per example, averaged over the
    batch. ``pos_scores`` is ``(B,)`` or ``(B, 1)``; ``neg_scores`` is
    ``(B, num_neg)``. Unlike :func:`bpr_loss` this couples all negatives in a
    single normaliser, which is what the full-catalog ranking metric rewards.
    """
    if pos_scores.dim() == 2:
