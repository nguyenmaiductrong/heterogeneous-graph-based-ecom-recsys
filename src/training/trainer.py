...
class TrainConfig:
    ...
    ranking_loss: str = "softmax"   # "softmax" (sampled-softmax CE) or "bpr"
    ...
    def from_yaml(cls, cfg: dict) -> "TrainConfig":
        ...
        return cls(
            ...
            ranking_loss=str(loss.get("ranking_loss", cls.ranking_loss)).lower(),
...
def train(
    ...
    _rank_desc = (
        "BPR" if cfg.ranking_loss == "bpr"
        else f"sampled-softmax(T={cfg.loss_temperature:.2f}, num_neg={cfg.num_neg})"
    )
    logger.info(
        "Loss: %s(alpha=%.2f, w_min=%.2f, weights=%s) + lambda_cl=%.3f + lambda_conv=%.3f + lambda_mono=%.3f + lambda_wd=%.1e",
        _rank_desc, cfg.bpr_alpha, cfg.bpr_w_min,
